...
const EVIDENCE_ROOT = path.join("e2e", "evidence", "busca-global-fase2-uat");
...
test.describe("UAT Busca global Fase 2", () => {
  test.setTimeout(240_000);

  test("sistemas, feed, UniLio e ramais na busca", async ({ browser, request }) => {
    const { stamp, runDir } = createEvidenceRun();
    const marker = `E2E-BUSCA-F2-${stamp}`;
    let passed = false;
    let errorMessage = "";
    ...
    try {
      ...
      const searchRes = await request.get(
        `${API_BASE_URL}/api/v1/search?q=a&limit=10&types=systems,feed,unilio,ramais`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      expect(searchRes.ok()).toBeTruthy();
      const payload = await searchRes.json();
      writeEvidence("00-search-fase2-api.json", payload);
      expect(payload).toHaveProperty("systems");
      expect(payload).toHaveProperty("feedPosts");
      expect(payload).toHaveProperty("ramais");
      expect(payload.uniLioCourses !== undefined || payload.unilioCourses !== undefined).toBeTruthy();

      const { context, page } = await openAuthedPage(browser, token);
      try {
        await page.goto(`${PAGE_BASE_URL}/busca?q=ramal`, { waitUntil: "networkidle" });
        await expect(page.getByTestId("global-search-page")).toBeVisible();
        await page.screenshot({ path: evidencePath("01-colaborador-busca-ramal.png"), fullPage: true });

        await expect(page.getByTestId("search-filter-systems")).toBeVisible();
        await expect(page.getByTestId("search-filter-feed")).toBeVisible();
        await expect(page.getByTestId("search-filter-unilio")).toBeVisible();
        await expect(page.getByTestId("search-filter-ramais")).toBeVisible();
        await page.screenshot({ path: evidencePath("02-colaborador-filtros-fase2.png"), fullPage: true });

        await page.getByTestId("global-search-page-input").fill("leo");
        await page.waitForTimeout(800);
        await page.screenshot({ path: evidencePath("03-colaborador-resultados-fase2.png"), fullPage: true });
      ...
      writeEvidence("99-uat-summary.md", [
        `# UAT Busca global Fase 2 — evidências`,
        ``,
        ...
        `## Fluxo validado`,
        ``,
        `1. API retorna systems/feed/unilio/ramais`,
        `2. Página /busca exibe filtros Fase 2`,
        `3. Busca com termo mostra resultados`,
        ``,
      ].join("\n"));
    }
  });
